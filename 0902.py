...
while not done:
    ...
    if abs(virsnake.x - virfood.x) + abs(virsnake.y - virfood.y) != 1:
        print('normal route')
        #蘋果不在正旁邊 一般流程 繼續找路
        while eatenVirApple == False:   #開始跑虛擬吃蘋果 while去一步一步得更新現在位置(虛擬的)
        ...
        print('food beside', end="")
        if abs(virfood.x - virsnake.x) == 1 and virfood.y - virsnake.y == 0:
            if virfood.x - virsnake.x == 1:
                virsnake.yV = 0
                virsnake.xV = 1
                print('   going right')
            else:
                ...
                virsnake.xV = -1
                print('   going left')
        if virfood.x - virsnake.x == 0 and abs(virfood.y - virsnake.y) == 1:
            if virfood.y - virsnake.y == 1:
                virsnake.xV = 0
                virsnake.yV = 1
                print('   going down')
            else:
